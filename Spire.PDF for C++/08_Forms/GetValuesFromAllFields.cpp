...
int main()
{
	...
	wstring inputFile = input_path + L"AllFields.pdf";
	wstring output_path = OUTPUTPATH;
	wstring outputFile = output_path + L"GetValuesFromAllFields.txt";
	...
	for (int i = 0; i < formWidget->GetFieldsWidget()->GetCount(); i++)
	{
		...
		if (wcscmp(newName2, field->GetInstanceTypeName()) == 0)
		{
			...
			intrusive_ptr<PdfListWidgetItemCollection> items = listBoxField->GetValues();
		...
		if (wcscmp(newName3, field->GetInstanceTypeName()) == 0)
		{
			...
			intrusive_ptr<PdfListWidgetItemCollection> items = comBoxField->GetValues();
		...
		{
			intrusive_ptr<PdfRadioButtonListFieldWidget> radioBtnField = Object::Convert<PdfRadioButtonListFieldWidget>(field);
