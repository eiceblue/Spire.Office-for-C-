...
void DrawPath(intrusive_ptr<PdfPageBase> page)
{
	intrusive_ptr<PointF> points[5];
	float PI = 3.1415926;
	for (int i = 0; i < 5; i++) {
	...
	intrusive_ptr<PdfPen> pen = new PdfPen(new PdfRGBColor(Spire::Common::Color::GetDeepSkyBlue()), 0.02f);
	intrusive_ptr<PdfBrush> brush1 = new PdfSolidBrush(new PdfRGBColor(Spire::Common::Color::GetCadetBlue()));
	...
	intrusive_ptr<PdfLinearGradientBrush> brush2 = new PdfLinearGradientBrush(new PointF(-2, 0),
	...
	intrusive_ptr<PdfRadialGradientBrush> brush3 = new PdfRadialGradientBrush(new PointF(0, 0), 0.f,
	...
	intrusive_ptr<PdfTilingBrush> brush4 = new PdfTilingBrush(new RectangleF(0, 0, 4.f, 4.f));
...
void DrawSpiro(intrusive_ptr<PdfPageBase> page)
{
	//Save graphics state
	intrusive_ptr<PdfGraphicsState> state = page->GetCanvas()->Save();
...
void DrawRectangle(intrusive_ptr<PdfPageBase> page)
{
	//Save graphics state
	intrusive_ptr<PdfGraphicsState> state = page->GetCanvas()->Save();

	intrusive_ptr<PdfPen> pen = new PdfPen(new PdfRGBColor(Spire::Common::Color::GetChocolate()), 1.f);
	page->GetCanvas()->DrawRectangle(pen, new RectangleF(new PointF(20, 310), new SizeF(150, 120)));
	page->GetCanvas()->Restore(state);

}

void DrawPie(intrusive_ptr<PdfPageBase> page)
{
	//Save graphics state
	intrusive_ptr<PdfGraphicsState> state = page->GetCanvas()->Save();
	intrusive_ptr<PdfPen> pen = new PdfPen(new PdfRGBColor(Spire::Common::Color::GetDarkRed()), 2.f);
	page->GetCanvas()->DrawPie(pen, 220, 320, 100, 90, 360, 360);
	page->GetCanvas()->Restore(state);

}

void DrawEllipse(intrusive_ptr<PdfPageBase> page)
{
	//Save graphics state
	intrusive_ptr<PdfGraphicsState> state = page->GetCanvas()->Save();
	intrusive_ptr<PdfBrush> brush = new PdfSolidBrush(new PdfRGBColor(Spire::Common::Color::GetCadetBlue()));
...
int main() {
	...
	wstring outputFile = output_path + L"DrawShape.pdf";
